 ...
 * Absolute URLs (Strapi configured with a CDN/PUBLIC_URL, or our own
 * `/public/images/*` fallbacks) are returned untouched.
 *
 * Host precedence:
...
/**
 * @param url Raw `.url` from a Strapi media object, a `photoUrl` string,
 *            or a local `/images/*` path. May be null/undefined.
 * @returns An absolute (or app-relative `/images/*`) URL, or null when
...
export function strapiMedia(url: string | null | undefined): string | null {
  ...
  // App-local asset shipped in /public (e.g. "/images/home-hero.jpg").
  // These resolve correctly against the Next origin, so don't rewrite.
  if (url.startsWith('/images/') || url.startsWith('/nhs')) return url;
